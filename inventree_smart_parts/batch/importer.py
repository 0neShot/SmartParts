...
def _parse_excel(file_obj) -> List[Dict[str, str]]:
    ...
    if 'mpn' not in normalized_headers:
        raise ValueError(
            f"No 'MPN' column found. Available columns: {headers}"
        )
...
def process_batch(
    ...
    thread = threading.Thread(
        target=_run_batch,
        args=(job, rows, plugin_instance, assembly_description, assembly_revision, assembly_category_id),
...
def _run_batch(
    ...
    for idx, row in enumerate(rows, start=1):
        try:
            ...
            job.results.append(BatchRowResult(
                row_number=idx,
                mpn=row.get('mpn', '?'),
                status='error',
                error=str(e),
    ...
    if job.assembly_name and bom_specs:
        ...
        if not asm_result.success:
            logger.error(
                f"Batch job {job.job_id} assembly failed: {asm_result.errors}"
            )
...
def _process_single_row(
    ...
    if cat_source:
        ...
        cat_id, cat_path, score = fuzzy_match_category(
            cat_source, threshold, default_cat,
